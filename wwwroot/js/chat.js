"use strict";

var connection = new signalR.HubConnectionBuilder().withUrl("/chatHub").build();

// id người mình sẽ gửi tin nhắn
var toUserID = null;
// người gửi tin nhắn
var fromUserID = localStorage.getItem("userLoggin");
//Disable the send button until connection is established.
document.getElementById("sendButton").disabled = true;

// trường hợp gửi cho tất cả
connection.on("ReceiveMessage", function (user, message) {
    var li = document.createElement("li");
    document.getElementById("messagesList").appendChild(li);
    li.textContent = `${user} says ${message}`;
});
// trường hợp gửi cho một đối tượng cụ thể
connection.on("ReceiveMessageOthor", function (userRenderID, userReceiverID, message) {
    if (fromUserID == userRenderID || (userReceiverID == fromUserID && userRenderID == toUserID)) {

        // căn phải cho thằng gửi và căn trái cho thằng nhận
        if (fromUserID == userRenderID) {
            
            //var p = document.createElement("p");
            //document.getElementById("showMessage").appendChild(p)
            //p.textContent = `${message}`
            //p.style.textAlign = "right"
            var child = `  <div class="d-flex justify-content-end mb-4">
                            <div class="msg_cotainer_send">
                                ${message}
                               
                            </div>
                            <div class="img_cont_msg">
                                <img src="data:image/jpeg;base64,"
                                    class="rounded-circle user_img_msg">
                            </div>
                        </div>`
            document.getElementById("msg_card_body").innerHTML += child
        }
        // (userReceiverID == fromUserID && userRenderID == toUserID)
        else {
            //var p = document.createElement("p");
            //document.getElementById("showMessage").appendChild(p)
            //p.textContent = `${message} `
            //p.style.textAlign = "left"
            var child = `  <div class="d-flex justify-content-start mb-4">
                            <div class="img_cont_msg">
                                <img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg"
                                    class="rounded-circle user_img_msg">
                            </div>
                            <div class="msg_cotainer">
                               ${message}
                               
                            </div>
                        </div>`
            document.getElementById("msg_card_body").innerHTML += child
        }
        
    }
    console.log("fromUserID", fromUserID, "userRenderID", userRenderID)
    console.log("toUserID", toUserID, "UserReceiverID", userReceiverID)
});

// sau khi kết nối thành công => do something....
connection.start().then(function () {
    document.getElementById("sendButton").disabled = false;
}).catch(function (err) {
    return console.error(err.toString());
});

document.getElementById("sendButton").addEventListener("click", function (event) {
    
    var message = document.getElementById("messageInput").value;
   
    // kiểm tra xem đã chọn người nào để gửi tin nhắn chưa
    if (toUserID) {
        console.log("dúng rồi" + fromUserID)
        fromUserID = localStorage.getItem("userLoggin")
        connection.invoke("SendMessageToReceiver", fromUserID, toUserID, message).catch(function (err) {
            return console.error(err.toString());
        });
        event.preventDefault();
    } else {
        console.log("sai rồi")
        connection.invoke("SendMessage", user, message).catch(function (err) {
            return console.error(err.toString());
        });
        event.preventDefault();
    }
    var message = document.getElementById("messageInput").value =""
    
    //
    
});

// chọn 1 người để trò chuyện
function sendMessageWithOthor(idRevices, emailRevices) {
    $('.col-md-8.col-xl-6.chat').css("display", "block")
    $('#user_info').html(emailRevices)
    // kiểm tra mình có đang hiện tại chat vs ngauowif này hay không

    // nếu đang là người đang chat hiện tại thì chát tiếp

    let idRevices1 = localStorage.getItem("toUser")
    if (toUserID != null && idRevices == idRevices1) {
            return; 
    }
    // xóa hết thông tin đang chát-- TH chọn chát vs người khác
    const list = document.getElementById("msg_card_body");
    while (list.hasChildNodes()) {
        list.removeChild(list.firstChild);
    }
    // nếu không thì tạo mới người này
    toUserID = idRevices
    // và lưu lại trên locol storage
    localStorage.setItem("toUser", toUserID);

    // gửi yêu cầu kiểm tra xem 2 đứa này đã nhắn tin vs nhau lần nào chưa

    // nếu rồi thì hiện thị tin nhắn của 2 đứa này
    $.ajax({
        type: "get",
        url: "/MessageContext/ListMessageContexts",
        dataType: "json",
        data: {
            userRenderID: fromUserID,
            userReceiverID: toUserID,
        },
        success: function (data) {
            console.log(data.data)
           
            
            for (let i = 0; i < data.data.length; i++) {


                if (data.data[i].userId == fromUserID) {
                    //var p = document.createElement("p");
                    //document.getElementById("showMessage").appendChild(p)
                    //p.textContent = `${data.data[i].messageContent}`
                    //p.style.textAlign = "right"
                    var child = `  <div class="d-flex justify-content-end mb-4">
                            <div class="msg_cotainer_send">
                                ${data.data[i].messageContent}
                                
                            </div>
                            <div class="img_cont_msg">
                                <img src="data:image/jpeg;base64,"
                                    class="rounded-circle user_img_msg">
                            </div>
                        </div>`
                    document.getElementById("msg_card_body").innerHTML += child
                }

                if (data.data[i].userId == toUserID) {
                    //var p = document.createElement("p");
                    //document.getElementById("showMessage").appendChild(p)
                    //p.textContent = `${data.data[i].messageContent}`
                    //p.style.textAlign = "left"

                    var child = `  <div class="d-flex justify-content-start mb-4">
                            <div class="img_cont_msg">
                                <img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg"
                                    class="rounded-circle user_img_msg">
                            </div>
                            <div class="msg_cotainer">
                               ${data.data[i].messageContent}
                               
                            </div>
                        </div>`
                    document.getElementById("msg_card_body").innerHTML += child
                }
              
            }
           

        },
        error: function (data, textStatus,
            errorThrown) {
            console.log("error", data)
        },
    });

    // nếu chưa thì thôi
    
}